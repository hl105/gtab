...
class GTAB:
    ...
    def __delete_all_internal_files(self):
        ...
        nl = '\n'
        tb = '\t'
        logging.info(f"This will delete the following files:\n\t{(nl + tb).join([os.path.basename(f) for f in files])}")
        c = input("Are you sure? (y/n): ")
        if c[0].lower() == 'y':
            logging.info("Deleting...")
            for f in files:
                os.remove(f)
            logging.info("Internal files deleted!")
        else:
    ...
    def __init__(self, dir_path=None, from_cli=False):
        ...
        if dir_path is None:
            ...
            if not os.path.exists(dir_path):
                ...
                logging.info("Directory already exists, loading data from it.")

        logging.info(f"Using directory '{self.dir_path}'")
        if from_cli:
    ...
    def _diagnose_bads(self, gres, keywords):
        ...
        logging.info(f"Total bad: {len(ret)}")
    ...
    def _get_google_results(self):
        ...
        if os.path.exists(fpath):
            ...
            if not os.path.exists(fpath_keywords):
                ...
                logging.info(f"Getting keywords from {fpath_keywords}")
    ...
    def print_options(self):
        ...
        if self.from_cli:
            logging.info(f"Config file: {os.path.join(self.dir_path, 'config', 'config_cl.json')}")
        else:
            logging.info(f"Config file: {os.path.join(self.dir_path, 'config', 'config_py.json')}")
    ...
    def set_options(self, pytrends_config=None, gtab_config=None, conn_config=None, overwite_file: bool = False):
        ...
        if overwite_file:
            ...
            logging.info(f"Overwriting config at: {config_path}\n")
    ...
    def delete_gtab(self, src, require_confirmation=True):
        ...
        if src.strip() == os.path.basename(self.active_gtab).strip():
            logging.info("The deleted gtab was set to active.")
            self.active_gtab = None
    ...
    def set_active_gtab(self, def_gtab):
        ...
        logging.info(f"Active anchorbank changed to: {os.path.basename(self.active_gtab)}\n")
